...
import tictim.paraglider.mixin.AbstractWidgetAccessor;
...
public class BargainScreen extends Screen implements DisableStaminaRender{
	...
	protected void renderBg(GuiGraphics guiGraphics){
		...
		if(!this.catalog.isEmpty()){
			...
			for(BargainButton button : this.buttons){
				if(((AbstractWidgetAccessor) (Object) button).isIsHovered()) renderPreview(guiGraphics, button.actualIndex());
	...
	private final class BargainButton extends AbstractButton{
		...
		private void renderTradeIndicator(GuiGraphics guiGraphics){
			...
			guiGraphics.blit(MERCHANT_GUI_TEXTURE, x+39,
					y+5,
					0,
		...
		private void renderItems(GuiGraphics guiGraphics){
			...
			for(int i = demands.size()-1; i>=0; i--){
				...
				int itemX = x+determineItemPosition(i, demands.size(), BUTTON_INPUT_X_OFFSET_START, BUTTON_INPUT_X_OFFSET_END);
				guiGraphics.renderFakeItem(stack, itemX, y+2);
				if(demand.quantity()!=1)
					guiGraphics.renderItemDecorations(font, stack, itemX, y+2, String.valueOf(demand.quantity()));
			}
			...
			for(int i = offers.size()-1; i>=0; i--){
				...
				int itemX = x+determineItemPosition(i, offers.size(), BUTTON_OUTPUT_X_OFFSET_START, BUTTON_OUTPUT_X_OFFSET_END);
				guiGraphics.renderFakeItem(stack, itemX, y+2);
				if(offer.quantity()!=1)
					guiGraphics.renderItemDecorations(font, stack, itemX, y+2, String.valueOf(offer.quantity()));
			}
		}
